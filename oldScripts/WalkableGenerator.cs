...
public class WalkableGenerator : MonoBehaviour {
	...
	void GenerateMeshHits(){
		...
		for (int i = 0; i <= max; i += 2) {
			...
			vertices.Add (layerHits[0].hit.point - layerHits[0].hit.normal * platform_height);
			...
			for(int j = 1; j < layerHits.Count; j++){				
				...
				//else add vertices
					...
					vertices.Add (layerHit.hit.point - layerHit.hit.normal * platform_height);
					...
					if (nextCol != null) {
						...
						vertices.Add (nextCol.hit.point - nextCol.hit.normal * platform_height);
					}
				}
			}
